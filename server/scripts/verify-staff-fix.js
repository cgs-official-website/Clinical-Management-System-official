async function run() {
  ...
  const preflightRes = await fetch('http://localhost:5000/api/admin/staff', {
    method: 'OPTIONS',
    headers: {
  ...
  const loginRes07 = await fetch('http://localhost:5000/api/auth/login', {
    method: 'POST',
  ...
  const loginRes007 = await fetch('http://localhost:5000/api/auth/login', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
